...
(() => {
  ...
  const licenseDetails = pkg => `<section class="inspector-section license-details"><h3>Declared licenses</h3><div class="chain-label">Publisher metadata per resolved version. These labels do not determine permitted use.${data.isDemo?' Demo license data is illustrative.':''}</div>${licenseEntries(pkg).map(({version,license})=>{
    ...
    return `<div class="license-detail"><strong>${esc(version)}</strong> ${pill(licenseLabel(license),license.kind==='expression'?'purple':'warning')}${license.kind==='file'?`<div class="chain-label">${esc(license.value)}</div>`:''}${url?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${license.kind==='expression'?'View license expression':'Open license link'} ↗</a>`:''}${license.requireAcceptance?`<div>${pill('Package requires license acceptance','warning')}</div>`:''}${license.note?`<div class="chain-label">${esc(license.note)}</div>`:''}</div>`;
  }).join('')}</section>`;
  ...
  function renderGraph() {
    ...
    for(const [key] of nodes) {
      ...
      const icon=svg('text',{x:12,y:25,fill:isProject||isRef?'#9d8bbf':'#6caa93','font-size':15});icon.textContent=isProject||isRef?'▧':'⬡';group.append(icon);
  ...
  function packageButton(id, version, requested=''){return `<button class="detail-row" data-package="${esc(id)}" title="${esc(id)}${requested?' · requested '+esc(requested):''}"><span>⬡ ${esc(id)}</span>${packageIndex.get(lower(id))?.hasVersionDrift?'<i class="drift-dot"></i>':''}<small>${esc(version)}</small></button>`;}
  function renderTable(){
    ...
    $('package-table').innerHTML=packages.map(p=>`<tr><td>${esc(p.id)}${p.hasVersionDrift?'<small>⇄ Version drift</small>':''}</td><td>${p.versions.map(v=>pill(v,p.hasVersionDrift?'warning':'good')).join('')||pill('Unresolved','warning')}${p.usages.some(u=>!u.resolved)?pill('Includes declarations','warning'):''}</td><td class="license-cell">${licenseBadges(p)}</td><td>${unique(p.usages.map(u=>u.project)).length}</td><td>${p.usages.some(u=>u.direct)?pill('Direct'):''}${p.usages.some(u=>u.resolved&&!u.direct)?pill('Transitive'):''}</td><td><button class="button" data-package="${esc(p.id)}">Inspect ↗</button></td></tr>`).join('');
